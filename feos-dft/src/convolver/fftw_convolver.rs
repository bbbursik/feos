...
#[derive(Clone)]
pub struct FFTWConvolver<T, D: Dimension> {
    /// k vectors
    k_abs: Array<f64, D>,
...
impl<T, D: Dimension + 'static> FFTWConvolver<T, D>
where
    ...
    <D::Larger as Dimension>::Larger: Dimension<Smaller = D::Larger>,
{
    ...
    // ) -> Arc<dyn Convolver<T, D>> {
        ...
        let mut axes_peek = axes.iter().peekable();
        while let Some(ax) = axes_peek.next(){
            // for ax in axes.iter().rev().skip(1).rev() {
            if axes_peek.peek().is_some(){
...
/*
impl<T: FftNum, D: Dimension> FFTWConvolver<T, D> {
...
impl<T, D: Dimension> Convolver<T, D> for FFTWConvolver<T, D>
where
    T: FftNum + DualNum<f64>,
    D::Larger: Dimension<Smaller = D>,
    <D::Larger as Dimension>::Larger: Dimension<Smaller = D::Larger>,
